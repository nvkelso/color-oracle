...
//  Tweaks by Bernie Jenny, April 2018:
//  * replaced exit(0) with return type NSString with the path to the moved app bundle to indicate the app needs to restart.
//  * changed question message to specific needs of Color Oracle.
//  * alert is not showing supression button.
//  * not trying to unmount disk image.
...
#define kStrMoveApplicationQuestionMessage _I10NS(@"Color Oracle needs to be in the Applications folder to be added to the menu bar when you log in.")
#define kStrMoveApplicationButtonMove _I10NS(@"Move to Applications Folder and Restart")
...
NSString* PFMoveToApplicationsFolderIfNecessary(void) {
	...
	if (IsInApplicationsFolder(bundlePath) && !isNestedApplication)
        return nil;
